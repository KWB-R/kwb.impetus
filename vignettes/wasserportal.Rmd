...
is_ghactions <- identical(Sys.getenv("CI"), "true") && 
  nchar(Sys.getenv("GH_ACTIONS_KWB_R")) > 0
...
markdown_link <- function(base_name) {
  markdown_link(
    name = base_name, 
    url = paste0("https://kwb-r.github.io/kwb.impetus/", base_name)
  )
}
```
...
surfacewater_level_master <- wp_data[[get_data_name(para_name)]] %>% 
  ...
  dplyr::mutate(
    ...
    Decade_Label = sprintf("%d - %d", .data$Decade, .data$Decade + 9),
...
decades <- tibble::tibble(
  names = sort(unique(surfacewater_level_master_agg$Decade_Label)),
  values = c('darkblue', 'blue', 'darkgreen', 'lightgreen', 'orange', 'red')
)

kwb.utils::preparePdf(
  sprintf("%s.pdf", para_name_export),
  landscape = TRUE,
  width.cm = 29.7, 
  height.cm = 21
)
...
for (id in ids) {
  ...
  gg <- p1 %>% 
    ...
    ggplot2::geom_ribbon(
      ggplot2::aes_string(ymin = "q05", ymax = "q95", fill = "Decade_Label"), 
      alpha = 0.1
    ) + 
    ...
    ggplot2::labs(
      y = "Monthly Mean Water Level (m NN)", 
      x = "Month Number", 
      col = "Mean", 
...
decades <- tibble::tibble(
  names = sort(unique(surfacewater_flow_master_agg$Decade_Label)),
  values = c('darkblue', 'blue', 'darkgreen', 'lightgreen', 'orange', 'red')
)

kwb.utils::preparePdf(
  sprintf("%s.pdf", para_name),
  landscape = TRUE,
  width.cm = 29.7, 
  height.cm = 21
)
...
for (id in ids) {
  ...
  gg <- p1 %>% 
    ...
    ggplot2::theme(legend.position="bottom") +
    ggplot2::labs(
      y = "Monthly mean Surface Water Flow (m\u00B3/s)", 
      x = "Month Number", 
      col = "Mean", 
      fill = "5%/95% Conf.-Interval",
      title = unique(p1$Label)
...
decades <- tibble::tibble(
  names = sort(unique(groundwater_level_master_agg$Decade_Label)),
  values = c('darkblue', 'blue', 'darkgreen', 'lightgreen', 'orange', 'red')
)
...
for (id in ids) {
  ...
  gg <- p1 %>% 
    ...
    ggplot2::theme(legend.position="bottom") +
    ggplot2::labs(
      y = "Monthly Mean Groundwater Level (m NN)", 
      x = "Month Number", 
      col = "Month", 
      fill = "5%/95% Conf.-Interval",
      title = unique(p1$Label)
...
+ `r markdown_link("surface-water_level.csv")`

+ `r markdown_link("surface-water_flow.csv")`


- **Groundwater**

+ `r markdown_link("groundwater_level.csv")`
...
+ `r markdown_link("surface-water_level.pdf")`

+ `r markdown_link("surface-water_flow.pdf")`


- **Groundwater**

+ `r markdown_link("groundwater_level.pdf")`
