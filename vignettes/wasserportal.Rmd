...
for (id in ids) {
  ...
  gg <- p1 %>% 
    ...
    ggplot2_theme() + 
    ggplot2_labels_monthly_mean("Pegelstand (m NN)", title= unique(p1$Label))
...
for (id in ids) {
  ...
  gg <- p1 %>% 
    ...
    ggplot2_facet_wrap_decade(ncol = length(unique(p1$Dekade_Label))) +
    ggplot2_theme() + 
    ggplot2_labels_monthly_mean("Durchfluss (m\u00B3/s)", title = unique(p1$Label))
...
for (id in ids) {
  ...
  gg <- p1 %>% 
    ...
    ggplot2_facet_wrap_decade(ncol = length(unique(p1$Dekade_Label))) +
    ggplot2_theme() +
    ggplot2_labels_monthly_mean("Grundwasserstand (m NN)", title = unique(p1$Label))
