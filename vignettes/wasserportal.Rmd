...
decades <- tibble::tibble(names = unique(groundwater_level_master_agg$Dekade_Label)[order(unique(groundwater_level_master_agg$Dekade_Label))],
                          values = c('darkblue', 'blue', 'darkgreen', 'lightgreen', 'orange', 'red'))



kwb.utils::preparePdf("groundwater_level.pdf",
                      landscape = TRUE,
                      width.cm = 29.7, 
                      height.cm = 21)
...
gg <- p1 %>% 
  ...
  ggplot2::theme_bw() + 
  ggplot2::theme(legend.position="bottom") +
  ggplot2::labs(y = "Monatsmittel Grundwasserstand (m NN)", 
                x = "Monatsnummer", 
                col = "Mittelwert", 
                fill = "5%/95% Konf.-intervall",
                title = unique(p1$Label))
