...
set_date_year_decade_month_columns <- function(data) {
  data %>% 
    dplyr::mutate(
      Datum = as.Date(.data$Datum),
      Jahr = as.integer(format(.data$Datum, "%Y")), 
      Dekade = kwb.impetus::floor_decade(.data$Jahr),
      Dekade_Label = decade_label(.data$Dekade),
      Monat = as.factor(format(.data$Datum, "%m"))
    )
}

decades_tibble <- function(decade_labels) {
  tibble::tibble(
    names = sort(unique(decade_labels)),
...
surfacewater_level_master <- wp_data[[get_data_name(para_name)]] %>% 
  ...
  set_date_year_decade_month_columns() %>%
...
surfacewater_level_master_agg <- surfacewater_level_master %>% 
  ...
  dplyr::summarise(
    ...
    q05 = numeric_quantile(.data$Tagesmittelwert_Pegelstand_mNN, 0.05),
    q95 = numeric_quantile(.data$Tagesmittelwert_Pegelstand_mNN, 0.95),
  )

decades <- decades_tibble(surfacewater_level_master_agg$Dekade_Label)
...
surfacewater_flow_master_agg <- surfacewater_flow_master %>%  
  ...
  dplyr::summarise(
    ...
    q05 = numeric_quantile(.data$Tagesmittelwert, 0.05),
    q95 = numeric_quantile(.data$Tagesmittelwert, 0.95),
  ) 

decades <- decades_tibble(surfacewater_flow_master_agg$Dekade_Label)
...
groundwater_level_master_agg <- groundwater_level_master %>%  
  ...
  dplyr::summarise(
    ...
    q05 = numeric_quantile(.data$Messwert, 0.05),
    q95 = numeric_quantile(.data$Messwert, 0.95),
  ) 

decades <- decades_tibble(groundwater_level_master_agg$Dekade_Label)
