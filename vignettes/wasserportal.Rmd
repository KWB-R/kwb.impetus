...
ggplot2_theme_and_labels <- function(what, title) {
  ggplot2::theme_bw() + 
    ggplot2::theme(legend.position = "bottom") +
    ggplot2_labels_monthly_mean(what, title)
}
```
...
for (id in ids) {
  ...
  gg <- p1 %>% 
    ...
    ggplot2_theme_and_labels("Pegelstand (m NN)", title = unique(p1$Label))
...
for (id in ids) {
  ...
  gg <- p1 %>% 
    ...
    #ggplot2::geom_boxplot() +
    ggplot2_facet_wrap_decade(ncol = length(unique(p1$Dekade_Label))) +
    ggplot2_theme_and_labels("Durchfluss (m\u00B3/s)", title = unique(p1$Label))
...
for (id in ids) {
  ...
  gg <- p1 %>% 
    ...
    #ggplot2::geom_boxplot() +
    ggplot2_facet_wrap_decade(ncol = length(unique(p1$Dekade_Label))) +
    ggplot2_theme_and_labels("Grundwasserstand (m NN)", title = unique(p1$Label))
