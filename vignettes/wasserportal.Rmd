...
get_master_name <- function(para_name) {
  para_name <- stringr::str_remove(para_name, "daily_") 
...
overview_names <- overview_names[overview_names != "surface_water.quality"]
...
para_name <- "daily_surface-water_water-level"
...
surfacewater_level_master <- wp_data[[get_data_name(para_name)]] %>% 
  ...
  dplyr::mutate(Tagesmittelwert_Pegelstand_m_NHN = as.numeric(.data$Pegelnullpunkt_m_NHN) + .data$Tagesmittelwert/100) %>%
  ### remove -777 for messstellennummer 5867000 (few values in 2000) resulted by step above
  dplyr::filter(.data$Tagesmittelwert_Pegelstand_m_NHN != -777) %>%
  dplyr::select(- .data$Pegelnullpunkt_m_NHN) %>% 
...
surfacewater_level_master_agg <- surfacewater_level_master %>% 
  dplyr::group_by(.data$Messstellennummer, 
                  ...
                  .data$Month, 
                  .data$Label) %>% 
  dplyr::summarise(Monatsmittelwert_Pegelstand_m_NHN = mean(.data$Tagesmittelwert_Pegelstand_m_NHN),
                   q05 = as.numeric(quantile(.data$Tagesmittelwert_Pegelstand_m_NHN, probs = 0.05)),
                   q95 = as.numeric(quantile(.data$Tagesmittelwert_Pegelstand_m_NHN, probs = 0.95)),
                   ) 
...
decades <- tibble::tibble(names = unique(surfacewater_level_master_agg$Decade_Label)[order(unique(surfacewater_level_master_agg$Decade_Label))],
                          values = c('brown','darkblue', 'blue', 'darkgreen', 'lightgreen', 'orange', 'red'))
...
p2 <- surfacewater_level_master %>% 
  ...
  dplyr::group_by(.data$Messstellennummer, 
                  ...
                  .data$Label) %>% 
  dplyr::summarise(Monatsmittelwert_Pegelstand_m_NHN = mean(.data$Tagesmittelwert_Pegelstand_m_NHN),
                   q05 = quantile(.data$Tagesmittelwert_Pegelstand_m_NHN, probs = 0.05),
                   q95 = quantile(.data$Tagesmittelwert_Pegelstand_m_NHN, probs = 0.95)) %>% 
  dplyr::filter(.data$Messstellennummer == id) 
...
gg <- p1 %>% 
  ggplot2::ggplot(mapping = ggplot2::aes(x = as.integer(.data$Month), 
                                         y = .data$Monatsmittelwert_Pegelstand_m_NHN,
...
para_name <- "daily_surface-water_flow"
...
para_name_export <- "surface-water_flow"

readr::write_csv(surfacewater_flow_master_agg, sprintf("%s.csv", para_name_export))
